...
def calculate_k_shot_accuracy(model, n, k):
    ...
    for t in range(number_of_trials):
        ...
        prototypes = []
        test_data = sample_data_for_n_way_k_shot(grouped_data, n, k + queries_per_trial)
        ...
        for c in range(n):
            for q in range(queries_per_trial):
                query_prediction = get_embedding(model.predict(np.array(query_set[c][q].reshape((1, 35, 35, 1)))), embedding_length)
...
print(calculate_k_shot_accuracy(model, 5, 5))
